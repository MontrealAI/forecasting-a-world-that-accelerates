...
def _chart(output: dict[str, Any]) -> str:
    ...
    ax.plot(pd.to_datetime([row["date"] for row in observations]), [row["value"] for row in observations], marker="o", linewidth=1.8, label="Observed")
    for scenario in output["scenarios"]:
        ...
        if scenario["kind"] == "base":
            lower = np.asarray([point["intervals"]["p80"]["lower"] for point in scenario["forecast_points"]], dtype=float)
            upper = np.asarray([point["intervals"]["p80"]["upper"] for point in scenario["forecast_points"]], dtype=float)
...
def render_html_report(output: dict[str, Any], destination: str | Path) -> Path:
    ...
    model_rows = []
    for fit in sorted(output["model_fit"]["fits"], key=lambda row: float("inf") if row["aicc"] is None else row["aicc"]):
        model_rows.append(
            [
