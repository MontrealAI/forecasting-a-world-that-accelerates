...
def public_experiment() -> dict:
    ...
    rows = sorted(
        ((name, record["n"], record["rmse_log"], record["smape"]) for name, record in hindcast.items() if record["n"]),
        key=lambda x: x[2],
    )
...
def synthetic_regime_experiment() -> dict:
    ...
    result = {
        ...
        "seed": 20260728,
        "known_transition_year": 2.0,
        "full": full,
        "restricted_stale_regimes": stale,
    }
...
def canonical_ablation_experiment() -> dict:
    ...
    results = canonical_ablation_suite(
...
def workflow_experiment() -> dict:
    ...
    result = analyze_workflow(
        ...
        payload["parallel_efficiency"],
        payload["coordination_coefficient"],
        payload["correlated_failure_penalty"],
    )
...
def summary_table(results: dict) -> None:
    ...
    best_public = min(
        ((name, rec["rmse_log"]) for name, rec in public.items() if rec["n"] and rec["rmse_log"] is not None),
        key=lambda item: item[1],
    )
    synthetic = results["synthetic"]["full"]
    best_synthetic = min(
        ((name, rec["rmse_log"]) for name, rec in synthetic.items() if rec["n"] and rec["rmse_log"] is not None),
        key=lambda item: item[1],
    )
    text = "\n".join(
        [
            r"\begin{tabular}{lll}",
            r"\toprule",
            r"Evaluation & Best model by log-RMSE & Score \\",
            r"\midrule",
            f"Public excerpt (illustrative) & {latex_escape(best_public[0].replace('_', ' '))} & {best_public[1]:.3f} "
            + r"\\",
            f"Synthetic regime shift & {latex_escape(best_synthetic[0].replace('_', ' '))} & {best_synthetic[1]:.3f} "
            + r"\\",
            r"\bottomrule",
            r"\end{tabular}",
        ]
...
def main() -> int:
    ...
    write_json(
        ROOT / "results/reference/experiment_index.json",
        {"generated_at": "2026-07-28T14:00:00-04:00", "experiments": list(results)},
    )
    return 0
